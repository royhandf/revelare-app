...
const BookTable = React.memo(({ items, currentPage, onDelete }) => {
          ...
          {items.map((item, index) => (
            <tr className="bg-white border-b hover:bg-gray-50" key={item.id}>
              <td className="font-medium text-gray-700">
                {(currentPage - 1) * items.length + index + 1}
              </td>
              ...
              <td>
                {item.editors
                  ? item.editors.split(", ").length > 3
                    ? item.editors.split(", ").slice(0, 3).join(", ") + " ..."
                    : item.editors
                  : "-"}
              </td>
      ...
      {isDetailModalOpen && selectedItem && (
        ...
        >
          <div className="p-4 md:p-5 space-y-4">
            <div className="mb-4">
              <h4 className="text-lg font-semibold mb-2 text-gray-900">ISBN</h4>
